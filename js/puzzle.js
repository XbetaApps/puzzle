...
function init(difficulty) {
    ...
    // قم بتوليد رقم عشوائي لاختيار صورة
    const randomImageIndex = Math.floor(Math.random() * 5) + 1; // تغيير 3 إلى عدد الصور المتاحة

    _img = new Image();
    _img.addEventListener('load', onImage, false);
    
    // استخدم رقم الصورة العشوائية لاختيار اسم الصورة
    _img.src = `img/puzzle${randomImageIndex}.jpg`;

    PUZZLE_DIFFICULTY = difficulty;
    // استمر في باقي الشيفرة كما هي
}
...
function initPuzzle(){
    ...
    createTitle("احفظ الصورة جيدا قبل الضغط عليها");
    buildPieces();
}
